...
import argparse
import sys
from helpers import catcher, dataperiod, datatodf, fetch, update
import dateutil.parser
import regex as re


parser = argparse.ArgumentParser(description='A Last.fm Bar Chart Race Maker.', epilog='Visit link for more info.')
parser.add_argument('-v', '--version', help='prints out current version', action='version', version='LICCHART v1')

parser.add_argument('source', help='str: your last.fm username or csv filename', nargs='?')

parser.add_argument('-a', '--api', help='str: adds/changes custom api key', metavar='KEY')
...
parser.add_argument('-s', '--strt', help='str: sets starting date (default: your first scrobble)', metavar='DATE')
parser.add_argument('-e', '--end', help='str: sets ending date (default: your last scrobble)', metavar='DATE')

parser.add_argument('-b', '--bars', type=int, default=20, help='int: sets how many artists will be visible on the chart (default: %(default)s)', metavar='AMT')
parser.add_argument('-l', '--length', type=float, help='float: sets how long your animation will be (default: dynamic value)', metavar='MIN')
parser.add_argument('-f', '--fps', type=int, default=30, help='int: more frames take more time to generate, but provide a smoother animation', metavar='FPS')
...
# Source input
if args.source == None:
    print("You haven't specified your source (nickname or CSV filename). Please, enter it now:")
    args.source = input()
    if not args.source:
        sys.exit(catcher(0))
